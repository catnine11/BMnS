...
--도서조회(장르별 전체조회) --회원
SELECT GENRE_NAME, THUMBNAIL , BOOK_CODE, STATUS_CODE
FROM (
		SELECT GENRE_CODE, GENRE_NAME, ISBN, TITLE, AUTHOR, PUBLISHER, PUBLISH_DATE, THUMBNAIL, BOOK_CODE, STATUS_CODE
...
SELECT b.BOOK_CODE, bs.BOOK_SEQ, STATUS_CODE,THUMBNAIL,
ISBN, TITLE, AUTHOR, GENRE_NAME, GENRE_CODE, PUBLISHER, TO_CHAR(PUBLISH_DATE,'YYYY-MM-DD') AS PUBLISH_DATE , 
CONTENT, INTRO, REVIEW, AUTHORINFO,
BORROW_STATUS, RETURN_DATE, RESERVE_STATUS
FROM  BOOK_STATUS bs
...
--예약현황조회(관리자의 전체회원)
SELECT bs.BOOK_SEQ , RESERVE_TITLE, bb.USER_ID , br.USER_ID
...
INSERT INTO BOOK_RESERVE
(RESERVE_SEQ, RESERVE_TITLE, RESERVE_DATE, RESERVE_STATUS, USER_ID, BOOK_SEQ)
VALUES(RESERVE_SEQ.NEXTVAL, '누구와 함께 일할 것인가', TO_DATE(CURRENT_DATE-3, 'YY-MM-DD'), 'Y' , 3, 60);
...
CREATE SEQUENCE BOOK_CODE START WITH 61 INCREMENT BY 1;
...
SELECT bi.BOOK_CODE, bs.BOOK_SEQ, STATUS_CODE,THUMBNAIL,
	...
	FROM BOOKINFO bi 
		JOIN BOOK_GENRE bg USING (GENRE_CODE) 
		JOIN BOOK_STATUS bs ON bi.BOOK_CODE = bs.BOOK_CODE 
		LEFT JOIN (SELECT * FROM BOOK_BORROW bb 
                            LEFT JOIN BOOK_RESERVE brr USING(BOOK_SEQ)
                           WHERE bb.BORROW_STATUS !='N' OR (bb.BORROW_STATUS ='N' AND brr.RESERVE_STATUS = 'Y') ) bb2
		 	ON bs.BOOK_SEQ = BB2.BOOK_SEQ 
		LEFT JOIN BOOK_RESERVE br 
		 	ON bs.BOOK_SEQ = br.BOOK_SEQ 
	WHERE bb2.BOOK_SEQ  IS NULL
